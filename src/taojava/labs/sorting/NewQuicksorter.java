    ...
    extends SorterBridge<T>
{
  ...
  */
  int[] partition(T pivot, T[] vals, Comparator<T> order, int lb, int ub)
  {
    ...
    // initialize s to  0
    int e = lb; // first unknown value
    while (ub > e)
      {
        if (order.compare(vals[e], pivot) < 0)
          {
            // move the element to the less than section
            Utils.swap(vals, e, lb);
            // move both boundaries up 1
            lb++;
            e++;
          } // if element is less than val
        else if (order.compare(vals[e], pivot) > 0)
          {
            // move the element to the greater than section
            Utils.swap(vals, e, ub - 1);
            // move the boundary down 1
            ub--;
          } // if element is greater than val
        else if (order.compare(vals[e], pivot) == 0)
          {
            // move the boundary up 1
            e++;
